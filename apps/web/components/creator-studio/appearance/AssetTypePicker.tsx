import { ImageIcon, Mountain, Shirt, Smile, UserRound, Video, type LucideIcon } from 'lucide-react';
...
const assetIcons: Record<AppearanceAssetType, LucideIcon> = {
  portrait: UserRound,
  'full-body': ImageIcon,
  expression: Smile,
  outfit: Shirt,
...
export function AssetTypePicker({ value, onChange }: AssetTypePickerProps) {
  return (
    <section>
      <p className="text-sm font-semibold text-white">What are you creating?</p>
        ...
        {appearanceAssetTypes.map((option) => {
          ...
          return (
            <button
              ...
              aria-pressed={active}
              className={`group flex min-h-28 items-start gap-3 rounded-[20px] border p-4 text-left transition duration-200 ${
                active
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="text-white/34 mt-1 block text-xs leading-5">
                  {option.description}
                </span>
              </span>
            </button>
