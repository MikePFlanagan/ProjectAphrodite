...
export function VisualPreviewCanvas({
  ...
  const jsonPrompt = useMemo(
    () => ({
      ...
      assetType,
      prompt: promptValues,
      characterLocks: enabledLocks.map(
        (lock) => lock.id,
      ),
      compiledPrompt,
    }),
      ...
      <header className="flex flex-col gap-4 border-b border-white/[0.08] p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-white/35">
            Test the complete creation flow without using paid AI credits.
          </p>
        </div>
      ...
      <div className="space-y-5 p-5">
        <div className="rounded-2xl border border-fuchsia-200/10 bg-fuchsia-300/[0.045] p-4">
          ...
          <p className="mt-1 text-xs leading-5 text-white/40">
            This preview uses randomly selected test imagery and may not
            represent the values entered in your prompt, selected asset type,
            or character locks.
          </p>
        </div>
            ...
            <div className="grid h-full place-items-center p-8 text-center">
              <div>
                ...
                <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-white/35">
                  Add appearance details, then generate a test preview.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          {status !== 'success' ? (
            <button
              ...
              className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-fuchsia-500 to-violet-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-fuchsia-950/30 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-35"
            >
              {status === 'generating' ? (
                <>
            ...
            <>
              <button
                ...
                onClick={handleGenerate}
                disabled={!canGenerate}
                className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl border border-white/[0.1] bg-white/[0.045] px-5 py-3 text-sm font-semibold text-white/75 transition hover:bg-white/[0.08] hover:text-white disabled:cursor-not-allowed disabled:opacity-35"
              >
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-2xl border border-white/[0.08] bg-white/[0.025] px-4 py-3 text-white/25"
              >
        ...
        {!compiledPrompt && (
          <p className="text-center text-xs text-amber-200/65">
            Add at least one prompt detail before generating.
          </p>
        )}
        ...
        <div className="overflow-hidden rounded-[20px] border border-white/[0.08] bg-black/25">
          <div className="flex flex-col gap-3 border-b border-white/[0.07] p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-white/35">
                Structured payload prepared for the future image provider.
              </p>
            </div>
