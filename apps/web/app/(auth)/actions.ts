...
/*export async function signupAction(_: AuthState, formData: FormData): Promise<AuthState> {
  ...
  if (await db.user.findUnique({ where: { email } })) return { error: 'An account already exists for this email.' };
...
export async function loginAction(_: AuthState, formData: FormData): Promise<AuthState> {
  ...
  if (!parsed.success) return { fieldErrors: parsed.error.flatten().fieldErrors };
  try { await signIn('credentials', { email: parsed.data.email, password: parsed.data.password, redirectTo: '/dashboard' }); }
  catch (error) { if (error instanceof AuthError) return { error: 'Email or password was not recognized.' }; throw error; }
  return {};
}
